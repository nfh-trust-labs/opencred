...
const CHANNEL_TO_METHOD: Record<string, string> = {
  ...
  [IPC_CHANNELS.SCHEMA_GENERATE]: "generateSchema",
  ...
  [IPC_CHANNELS.PKCS11_PICK_LIBRARY]: "pkcs11PickLibrary",
  ...
  [IPC_CHANNELS.DEDI_PUBLISH_KEY]: "dediPublishKey",
  [IPC_CHANNELS.DEDI_SET_KEY_STATUS]: "dediSetKeyStatus",
...
describe("Preload API completeness", () => {
  ...
  it("should have all mapped methods present on a conforming API object", () => {
    ...
    const api: OpenCredDesktopAPI = {
      ...
      generateSchema: async () => ({ schema: {}, fields: [] }),
      ...
      pkcs11PickLibrary: async () => ({ success: false }),
      ...
      credentialHistoryAdd: async (
        req: Parameters<OpenCredDesktopAPI["credentialHistoryAdd"]>[0],
      ) => ({ ...req, id: "test", issuedAt: "2025-01-01T00:00:00Z" }),
      credentialHistoryDelete: async () => ({ deleted: true }),
      schemaFetchUrl: async () => ({ success: true }),
      customSchemaList: async () => ({ schemas: [] }),
      customSchemaSave: async () => ({
        id: "test",
        name: "test",
        schema: {},
      ...
      dediPublishKey: async () => ({ success: true }),
      dediSetKeyStatus: async () => ({ success: true }),
