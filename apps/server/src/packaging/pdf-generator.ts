...
export async function generatePdf(
  ...
  const footerText = customization?.footerText ?? "This certificate was generated by OpenCred Desktop. The credential is digitally signed and can be independently verified.";
    ...
    try {
      ...
      if (logoUri) {
        try {
          ...
          doc.y += logoH + 10;
        } catch { /* skip if logo data is invalid */ }
      }
        ...
        for (const [key, value] of subjectEntries) {
          if (typeof value === "object" && value !== null) {
            ...
            for (const [subKey, subValue] of Object.entries(value as Record<string, unknown>)) {
              drawLabelValue(doc, `  ${formatLabel(subKey)}`, String(subValue ?? ""), 130, labelColor, textColor);
            }
          } else {
      ...
      drawLabelValue(doc, "Verification Method", credential.proof.verificationMethod, 120, labelColor, textColor);
      ...
      if (Array.isArray(proof.x5c) && proof.x5c.length > 0) {
        drawLabelValue(doc, "Certificate Chain", `${proof.x5c.length} certificate(s) embedded`, 120, labelColor, textColor);
      }
      ...
      if (sealUri) {
        ...
        } catch { /* skip if seal data is invalid */ }
      }
