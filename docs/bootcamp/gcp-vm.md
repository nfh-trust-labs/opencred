...
| `500 CRYPTO_ERROR — JSON-LD canonicalization rejected the credential (strict mode): …` on `data-integrity` / `jws-2020` | A `credentialSubject` field that no JSON-LD context defines, or an `id` / URI-typed value that is a bare string (e.g. a meter number) | The message names the field or value. Remove/rename the field or pass an `inlineContext` that defines it; make identifiers absolute URIs (`urn:…`, `did:…`, `https://…`); or issue with `vc-jwt` |
