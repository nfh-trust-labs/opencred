...
| `pinned-fetch.ts` | `fetchWithPinnedIp(url, addresses, opts)` — DNS-rebinding-safe HTTPS GET that connects only to pre-validated IPs while keeping the hostname for TLS validation |
