...
* **Cross-replica work stealing — `inline` mode only.** In the default `inline` dispatch the actual signing for a batch is pinned to the replica that received the POST. If that replica dies mid-batch, the job is marked `interrupted` in Redis on graceful shutdown; otherwise the entry expires via TTL. There is no automatic re-issuance — clients should re-submit interrupted batches. The opt-in `queue` dispatch (see below) lifts this restriction by moving signing into a separate worker fleet.
