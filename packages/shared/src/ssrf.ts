...
function isPrivateIPv4(ip: string): boolean {
  ...
  // 198.18.0.0/15 (benchmark testing, RFC 2544) — 198.18.0.0 - 198.19.255.255
  if (ip.startsWith("198.")) {
    const secondOctet = parseInt(ip.split(".")[1], 10);
    if (secondOctet === 18 || secondOctet === 19) return true;
  }

  // 224.0.0.0/4 (multicast, RFC 5771) and 240.0.0.0/4 (reserved, RFC 1112)
  // First octet 224-255 covers both ranges.
