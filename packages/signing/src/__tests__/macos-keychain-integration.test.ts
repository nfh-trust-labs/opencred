/**
 * Integration tests for the macOS Keychain native addon.
 *
 * These tests are gated to only run on macOS (darwin). They verify that
 * the native addon can be loaded and exercised against the real macOS
 * Keychain. The test environment may have zero signing certificates,
 * so certificate-dependent tests are skipped when the keychain is empty.
 *
...
function tryLoadAddon() {
  try {
    ...
    return require("../../../native/build/Release/macos-keychain.node");
  } catch {
    return null;
  }
}

describe.skipIf(!IS_MACOS)("macOS Keychain Native Addon — Integration", () => {
  const addon = IS_MACOS ? tryLoadAddon() : null;

  it("should load the native addon without errors", () => {
    // The addon may not be built yet in CI; skip gracefully
    if (!addon) {
  ...
  it("should list certificates (may be empty)", () => {
    ...
    for (const cert of certs) {
      ...
      // Verify thumbprint is 64-char hex (SHA-256)
  ...
  it("should get public key for first available certificate", async () => {
    ...
    const certs = addon.listSigningCertificates();
    if (certs.length === 0) {
      console.log("No signing certificates in Keychain — skipping public key test");
      return;
    }
    ...
    // EC compressed key starts with 0x02 or 0x03
    // RSA SPKI starts with 0x30 (SEQUENCE)
